...
BAD_FILES=(`grep -E "android\.webkit\.|android\.net\." . -rl | grep "\.kt$" | grep -v ${ignoreFolder}`)
...
ALL_JAVA_FILES=(`find . -name "*.kt" | grep -v ${ignoreFolder}`)
for file in ${BAD_FILES_XML[@]}; do
	...
	for viewId in  ${viewIds[@]}; do
		#echo ${viewId}
		foundInJava=`grep -E "\b${viewId}\b" . -rl | grep -E "\.kt$"`
