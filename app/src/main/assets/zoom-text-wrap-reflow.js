...
`;
  ...
  function onPointerUp(ev) {
    ...
    if (
      !_touchDowns.length || // wtf?
      !(found = _touchDowns.find((item) => item.id === ev.pointerId)) || // wtf?
      (foundScrollOrPinchZoom =
        found.x !== 0 && found.y !== 0 && ev.x === 0 && ev.y === 0) // pinch zoom (dual pointer) or scroll (single pointer)
    ) {
    ...
    const element =
      firstNearestCommonAncestor(_touchDowns.map((item) => item.target)) ||
    ...
    document.documentElement.style.setProperty(
      "--text-reflow-wrap-max-width-var",
      `${maxWidth}px`,
    );
