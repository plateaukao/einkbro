...
import info.plateaukao.einkbro.activity.BookmarksReorderActivity
...
class BookmarkViewModel(private val bookmarkManager: BookmarkManager) : ViewModel() {
    ...
    fun showReorderUi(activity: Activity) {
        BookmarksReorderActivity.start(activity, currentFolder.value.id)
    }
