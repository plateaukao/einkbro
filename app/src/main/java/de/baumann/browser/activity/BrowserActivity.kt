...
class BrowserActivity : AppCompatActivity(), BrowserController, View.OnClickListener {
    // Menus
    private lateinit var menu_closeTab: View
    ...
    private fun showOverview() {
        ...
        val window = bottomSheetDialog_OverView!!.window
        ...
        Handler().postDelayed({ tab_ScrollView!!.scrollTo(currentAlbumController!!.albumView.left, 0) }, 250)
    }
    ...
    private fun initOverview() {
        ...
        open_startPage.setOnClickListener(View.OnClickListener {
            ...
            gridView.setOnItemLongClickListener(OnItemLongClickListener { parent, view, position, id ->
                show_contextMenu_list(gridList[position].title, gridList[position].url,
                        null, null, 0,
        ...
        open_history.setOnClickListener(View.OnClickListener {
            ...
            listView.setOnItemLongClickListener(OnItemLongClickListener { parent, view, position, id ->
                show_contextMenu_list(list[position].title, list[position].url, adapter, list, position,
    ...
    private fun initBookmarkList() {
        ...
        val adapter: SimpleCursorAdapter = object : SimpleCursorAdapter(context, layoutStyle, row, column, xml_id, 0) {
    ...
    private fun show_dialogFastToggle() {
        ...
        if (sp!!.getBoolean(getString(R.string.sp_javascript), true)) {
            sw_java.isChecked = true
        } else {
            sw_java.isChecked = false
        }
        if (sp!!.getBoolean(getString(R.string.sp_ad_block), true)) {
            sw_adBlock.isChecked = true
        } else {
            sw_adBlock.isChecked = false
        }
        if (sp!!.getBoolean(getString(R.string.sp_cookies), true)) {
            sw_cookie.isChecked = true
        } else {
            sw_cookie.isChecked = false
        }
    ...
    private fun toast_login(userName: String, passWord: String) {
        try {
            ...
            val copyUN = PendingIntent.getBroadcast(context, 0, copy, PendingIntent.FLAG_CANCEL_CURRENT)
            val intentFilter2 = IntentFilter("pwCopy")
            registerReceiver(pwCopy, intentFilter2)
            val copy2 = Intent("pwCopy")
            val copyPW = PendingIntent.getBroadcast(context, 1, copy2, PendingIntent.FLAG_CANCEL_CURRENT)
    ...
    private fun showOverflow(): Boolean {
        ...
        menu_shareCP.setOnClickListener(View.OnClickListener {
            hideBottomSheetDialog()
            val clipboard = getSystemService(CLIPBOARD_SERVICE) as ClipboardManager
